...
# USAGE_TO_AD="$(tput bold)\n\trecord ([-start] [-ab] [-rd] [-ad] [-md] [-fn[myscript[%d].sh]] [-end])$(tput sgr0)
#     \n\t$(tput bold)-fn$(tput sgr0)\t-\tDefault script filename myscript{date}.sh This is file name of the \n\t\t\trecodring script.
#     \n\t$(tput bold)-md$(tput sgr0)\t-\tIt will undo ad. i.e., it will turn off the auto directory recording.
#     \n\t$(tput bold)-ad$(tput sgr0)\t-\tIt will record the working directory of all the commands automatically.\n\t\t\tThis voids recdir command"
ST="NO"
AD="NO"
FN=""
